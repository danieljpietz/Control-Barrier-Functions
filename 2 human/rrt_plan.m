classdef rrt_plan < handle
    ...
    properties
        ...
        T_s                      %Sample time
        T_path_seg = 2.1;            %path segment duration
        ...
        time_mult = 0;          %multiplier of time in node selection
    ...
    methods
        ...
        function G = planning(obj,t_start,robot,obstacle_vec, max_pred_t)
           ...
           for kk=1:obj.num_obs
               if ~isempty(obstacle_vec.current_pose(:,kk))
                   ...
                   if Dist <obj.UnsafeRadius
                      ...
                      if hr_min > obj.Unsafes{kk}(robot.current_state,  obstacle_vec.current_pose(:,kk), obstacle_vec.current_r(kk))
                          hr_min = obj.Unsafes{kk}(robot.current_state,  obstacle_vec.current_pose(:,kk), obstacle_vec.current_r(kk));
                      end
                   end
               end
           end
           ...
           while (G.node_indx <= obj.max_node)   %tree expansion
                   ...
                   if length(xr_o{kk})>1
                       if ~isempty(xr_o{kk}{end-1})
                           ...
                           if Dist <obj.UnsafeRadius
                              ...
                              if hr_min>obj.Unsafes{kk}(xr(:,end-1),  xr_o{kk}{end-1}, xr_o_radii{kk}{end-1})
                                  hr_min = obj.Unsafes{kk}(xr(:,end-1),  xr_o{kk}{end-1}, xr_o_radii{kk}{end-1});
                              end
                           end
                       end
                   else
                       if ~isempty(xr_o{kk}{end})
                           ...
                           if Dist <obj.UnsafeRadius
                              ...
                              if hr_min>obj.Unsafes{kk}(xr(:,end),  xr_o{kk}{end}, xr_o_radii{kk}{end})
                                  hr_min = obj.Unsafes{kk}(xr(:,end),  xr_o{kk}{end}, xr_o_radii{kk}{end});
                              end
                           end
                       end
                   end
               ...
               G.node(G.node_indx).hr_min = hr_min;
               for i=1:obj.num_obs
